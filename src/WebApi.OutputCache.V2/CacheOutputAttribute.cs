...
namespace WebApi.OutputCache.V2
{
    ...
    public class CacheOutputAttribute : ActionFilterAttribute
    {
        ...
        protected static MediaTypeHeaderValue DefaultMediaType =
            new MediaTypeHeaderValue("application/json") {CharSet = Encoding.UTF8.HeaderName};
        ...
        /// <summary>
        ///     Corresponds to MustRevalidate HTTP header - indicates whether the origin server requires revalidation of a cache
        ///     entry on any subsequent use when the cache entry becomes stale
        /// </summary>
        ...
        protected virtual MediaTypeHeaderValue GetExpectedMediaType(HttpConfiguration config,
            HttpActionContext actionContext)
        {
            ...
            if (negotiator != null && returnType != typeof(HttpResponseMessage) &&
                (returnType != typeof(IHttpActionResult) || typeof(IHttpActionResult).IsAssignableFrom(returnType)))
            {
                ...
                if (actionContext.Request.Headers.Accept != null)
                {
                    ...
                    if (responseMediaType == null ||
                        !config.Formatters.Any(
                            x => x.SupportedMediaTypes.Any(value => value.MediaType == responseMediaType.MediaType)))
                        return DefaultMediaType;
                }
            }
