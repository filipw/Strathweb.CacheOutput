...
namespace WebApi.OutputCache.V2
{
    ...
    public sealed class AutoInvalidateCacheOutputAttribute : BaseCacheAttribute
    {
        ...
        public override void OnActionExecuted(HttpActionExecutedContext actionExecutedContext)
        {
            ...
            var actions = FindAllGetMethods(controller.ControllerType,
                TryMatchType ? actionExecutedContext.ActionContext.ActionDescriptor.GetParameters() : null);
            ...
            foreach (var action in actions)
            {
                var key = config.CacheOutputConfiguration()
                    .MakeBaseCachekey(controller.ControllerType.FullName, action);
                if (WebApiCache.Contains(key))
                    WebApiCache.RemoveStartsWith(key);
            }
        }

        private static IEnumerable<string> FindAllGetMethods(Type controllerType,
            IEnumerable<HttpParameterDescriptor> httpParameterDescriptors)
        {
            var actions =
            ...
            if (httpParameterDescriptors != null)
            {
                ...
                var filteredByType = filteredActions.ToList().Where(x =>
                {
                    ...
                    if (allowedTypes.Any(s => typeof(IEnumerable).IsAssignableFrom(x.ReturnType) &&
                                              x.ReturnType.GetGenericArguments().Any() &&
                                              x.ReturnType.GetGenericArguments()[0] == s)) return true;
                    if (allowedTypes.Any(s => typeof(IEnumerable).IsAssignableFrom(x.ReturnType) &&
                                              x.ReturnType.GetElementType() == s)) return true;
                    return false;
                });
